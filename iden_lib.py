...
def rap_run(metadata, appcom):
    ...
    if(appcom[0] == "print_ui"):
        ...
        if appcom[4] == "default":
            ...
            lbl.pack()
            if appcom[6] != "false":
                lbl = Label(window, text=_read_metadata(metadata, appcom[6]), font=("Arial Bold", 15))
                lbl.pack()
        ...
        if appcom[4] == "yae":
            ...
            if appcom[6] != "false":
                lbl = Label(window, text=_read_metadata(metadata, appcom[6]), font=("Arial Bold", 12))
...
def os_ui():
	...
	btn = Button(window, text=("""Iden Meta
